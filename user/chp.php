...
<div class="container-fluid py-5">
    <div class="container pt-5 pb-3">
        ...
        if (isset($_POST['change'])) {
            ...
            $npwd = $_POST['npwd'];
            $cpwd = $_POST['cpwd'];
            $sql = mysqli_query($con, "SELECT * from user where uid='$uid' and password='$opwd'") or die(mysqli_error($con));
            $nrows = mysqli_num_rows($sql);
            if ($nrows == 1) {
                if ($npwd == $cpwd) {
                    $qry = mysqli_query($con, "UPDATE user set password='$npwd' where uid='$uid'") or die(mysqli_error($con));
                    if ($qry) {
